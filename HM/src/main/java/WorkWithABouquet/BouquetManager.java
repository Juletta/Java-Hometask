 ...
 private static final Logger logger = Logger.getLogger(BouquetManager.class);
  ...
  private Double costFlowers;
   ...
   public BouquetManager()
   {
    ...
    public Double getCostBouquet() {
        return costBouquet;
    }

    public void setCostBouquet(Double costBouquet) {
    ...
    public void setCostFlowers(Double costFlowers) {
        this.costFlowers = costFlowers;
    }

    public Double costCalculation(FormBouquet formBouquet)
    {
        if (formBouquet==null){ logger.warn("Bouquet is not formed!");
        return 0.0;}                                                             //considers the full cost of the bouquet(flowers+tape+packing)
        else{
        ...
        logger.info("Cost bouquet-"+(double)Math.round(costBouquet*100d)/100);setCostBouquet(costBouquet);
        return costBouquet;}
    }

    public void sortByFreshness(FormBouquet formBouquet)
    {                                                               //sorts the flowers by freshness
        if (formBouquet==null) {
            ...
            for (int i = formBouquet.getBouquet().getNamesFlowers().length - 1; i > 0; i--) {                        //flower bubble sorting
                for (int j = 0; j < i; j++) {
                    if (formBouquet.getBouquet().getNamesFlowers()[j].getNumberDelivary() > formBouquet.getBouquet().getNamesFlowers()[j + 1].getNumberDelivary()) {
                        int tmp = formBouquet.getBouquet().getNamesFlowers()[j].getNumberDelivary();
                        formBouquet.getBouquet().getNamesFlowers()[j].setNumberDelivary(formBouquet.getBouquet().getNamesFlowers()[j + 1].getNumberDelivary());
                        formBouquet.getBouquet().getNamesFlowers()[j + 1].setNumberDelivary(tmp);
                    }
                }
            }
    ...
    public void flowerSearch(FormBouquet formBouquet, int a, int b)
    {                                                               //search for flowers by stem length
        if (formBouquet==null) {
            logger.warn("Bouquet is not formed!");
        }
        else {
